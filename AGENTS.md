...
3. **Build.** Build a calm, readable, sleek static site from `source/notes.md` and `source/site.md`. The site should make the protocol easy to understand. Organization is the agent's job. Wording on the site must stay faithful to the captured protocol.
